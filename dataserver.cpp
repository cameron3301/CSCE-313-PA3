...
void handle_process_loop(FIFORequestChannel *_channel, int bufsize);
vector<string> all_data [NUM_PERSONS];


void process_newchannel_request (FIFORequestChannel *_channel, int bufsize)
{
	nchannels++;
	...
	thread thread_for_client (handle_process_loop, data_channel, bufsize);
...
void process_file_request (FIFORequestChannel* rc, char* request, int bufsize){
...
int process_request(FIFORequestChannel *rc, char* _request, int bufsize)
{
	...
	if (m == DATA_MSG){
		...
		process_file_request (rc, _request, bufsize);
			
	}else if (m == NEWCHANNEL_MSG){
		process_newchannel_request(rc, bufsize);
	}
	else{
...
void handle_process_loop(FIFORequestChannel *channel, int bufsize)
{
	for (;;){
		...
		if (m == QUIT_MSG)
			break;
		process_request(channel, buffer, bufsize);
		delete [] buffer;
	}
}
...
{
	srand(time_t(NULL));
	int bufsize = atoi(argv[0]); // modify this to accept bufsize m from the client side
	...
	handle_process_loop(control_channel, bufsize);
